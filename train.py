...
def run_train_phase(model, P, Z, logger, epoch, phase):
    ...
    # test_sample = random.sample(range(len(Z['dataloaders'][phase])), 10)
    # idx = 0
    ...
    for batch in Z['dataloaders'][phase]:
        ...
        # if idx in test_sample:
        #     # image tensor is shape 
        #     print("image tensor {}".format(batch['image']))
        #     # print("Shape of image tensor {}".format(batch['image'].shape))
        #     # plt.imshow(  batch['image'].permute(1, 2, 0)  )
        #     # print("LABELS: {}".format(batch['labels_np']))
        # idx += 1
