...
'''
# TODO: rename file now that it is general and not only size bias
...
def observe_bias(label_matrix, imID_to_catWeights, image_list_ph, num_observations=1):
    '''
...
if __name__ == '__main__':
    ...
    (imID_to_catWeights, imID_to_annWeights) = get_weights_per_im(imID_to_anns, bias)
    ...
    if mode == 'test':
        ...
        for i in range(1, N + 1):
            for phase in ['train', 'val']:
                ...
                random.seed(SEED)
                biased_matrix = observe_bias(label_matrix, imID_to_catWeights, image_list[phase])
                np.save(os.path.join(base_path, 'formatted_{}_{}_{}_labels_obs.npy'.format(phase, bias, i)), biased_matrix)
...
#             with open(os.path.join(data_path + '/coco', 'VOCdevkit', 'VOC2012', 'ImageSets', 'Main', cat + '_' + phase + '.txt'), 'r') as f:
#                 for line in f:
#                     cur_line = line.rstrip().split(' ')
#                     image_id = cur_line[0]
#                     label = cur_line[-1]
...
#                 np.save(os.path.join(base_path, 'formatted_{}_labels_obs.npy'.format(phase)), biased_matrix)
